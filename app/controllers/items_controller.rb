class ItemsController < ApplicationController
  ...
  def index
    ...
    @mens = Item.ransack(by_category_id: 2).result.order(created_at: "DESC").limit(4)
    @kids = Item.ransack(by_category_id: 3).result.order(created_at: "DESC").limit(4)
    @janel = Item.ransack(brand_id_eq: 1).result.order(created_at: "DESC").limit(4)
    @biton = Item.ransack(brand_id_eq: 3).result.order(created_at: "DESC").limit(4)
    @shrimp = Item.ransack(brand_id_eq: 4).result.order(created_at: "DESC").limit(4)
    @mike = Item.ransack(brand_id_eq: 2).result.order(created_at: "DESC").limit(4)
  end
